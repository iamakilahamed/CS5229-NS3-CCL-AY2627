...
# This script runs the "multiple 1D ring" AllReduce pattern: within each ToR pod (32 hosts/pod
# for 128 hosts), hosts are grouped by "index mod 32" into 32 independent rings (e.g. {0,32,64,96},
# {1,33,65,97}, ...), each doing its own 1D ring-AllReduce across the 4 pods. This pattern is
# driven by the --comm-group-configuration file below (see inputs/comm_group/128host_32ring.json)
...
WORKLOADS=( \
  ${SCRIPT_DIR:?}"/../../inputs/workload/microbenchmark_allreduce/128host_32ring_32mb_1pass/job" \
  ${SCRIPT_DIR:?}"/../../inputs/workload/microbenchmark_allreduce/128host_32ring_64mb_1pass/job" \
  ${SCRIPT_DIR:?}"/../../inputs/workload/microbenchmark_allreduce/128host_32ring_128mb_1pass/job" \
  ${SCRIPT_DIR:?}"/../../inputs/workload/microbenchmark_allreduce/128host_32ring_256mb_1pass/job" \
  #   ${SCRIPT_DIR:?}"/../../inputs/workload/microbenchmark_allreduce/128host_32ring_256mb_4pass/job" \
)

WORKLOAD_NAMES=( \
  "multiring_32ring_32mb_128nodes" \
  "multiring_32ring_64mb_128nodes" \
  "multiring_32ring_128mb_128nodes" \
  "multiring_32ring_256mb_1pass_128nodes" \
  #   "multiring_32ring_256mb_4pass_128nodes" \
)

# The communicator-group file that partitions the 128 hosts into 32 independent rings.
...
COMM_GROUP_CONFIG="${SCRIPT_DIR:?}"/../../inputs/comm_group/128host_32ring.json

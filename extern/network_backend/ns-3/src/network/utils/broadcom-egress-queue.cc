...
namespace ns3 {
	...
	BEgressQueue::BEgressQueue() : Queue(), NS_LOG_TEMPLATE_DEFINE("BEgressQueue") {
	...
	Ptr<Packet> BEgressQueue::DoDequeueRR(bool paused[]) { //this is for switch only 
		NS_LOG_FUNCTION(this);
		...
		if (m_queues[0]->GetNPackets() > 0){ //0 is the highest priority
			...
			if (!found) {
				for (qIndex = 1; qIndex <= qCnt; qIndex++) {
					if (!paused[(qIndex + m_rrlast) % qCnt] && 
						m_queues[(qIndex + m_rrlast) % qCnt]->GetNPackets() > 0) { //round robin
						found = true;
						break;
					}
				}
	...
	Ptr<Packet> BEgressQueue::DequeueRR(bool paused[]) {
	...
	Ptr<Packet> BEgressQueue::DoDequeue(void) {
		NS_ASSERT_MSG(false, "BEgressQueue::DoDequeue not implemented, you should explicitly call");
		return 0;
	}

	//DoPeek doesn't work for multiple queues!!
	Ptr<const Packet> BEgressQueue::DoPeek(void) const {
		//std::cout << "Warning: Call Broadcom queues without priority\n";
		NS_LOG_FUNCTION(this);
	...
	uint32_t BEgressQueue::GetNBytes(uint32_t qIndex) const {
		return m_bytesInQueue[qIndex];
	}


	uint32_t BEgressQueue::GetNBytesTotal() const {
		return m_bytesInQueueTotal;
	}

	uint32_t BEgressQueue::GetLastQueue() {
		return m_qlast;
	}

	//********************************** 
	// Customized Queueing Implementation
	//**********************************
	Ptr<Packet> BEgressQueue::DequeueCustomized() {
		return DoDequeueCustomized();
	}

	// If you want to customize it, you can implement it here.
	Ptr<Packet> BEgressQueue::DoDequeueCustomized() {
		NS_ASSERT_MSG(false, "Customized Dequeue is not implemented yet");
		return 0;
	}

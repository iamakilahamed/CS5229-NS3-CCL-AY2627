...
namespace ns3 {
	...
	bool SwitchMmu::CheckIngressAdmission(uint32_t port, uint32_t qIndex, uint32_t psize){
		// Record the received packet size in Settings::switch_workload_statistics
		Settings::switch_workload_statistics[node_id] += psize;
	...
	bool SwitchMmu::CheckEgressAdmission(uint32_t port, uint32_t qIndex, uint32_t psize){
		// Assuming if there is space in ingress, there is space in egress
		return true;
	}

	void SwitchMmu::UpdateIngressAdmission(uint32_t port, uint32_t qIndex, uint32_t psize){
		Settings::switch_workload_statistics_per_port_ingress[node_id][port] += psize;
	...
	uint128_t SwitchMmu::GetFlowKey(uint32_t ip1, uint32_t ip2, uint16_t port1, uint16_t port2) {
		uint128_t ret = 0;
		ret |= (uint128_t(ip1) << 96); // ip1
		ret |= (uint128_t(ip2) << 64); // ip2
		ret |= (uint128_t(port1) << 32); // port1
		ret |= (uint128_t(port2) << 0); // port2
		return ret;
	}

	std::tuple<uint32_t, uint32_t, uint16_t, uint16_t> SwitchMmu::GetFlowTuple(uint128_t flowkey) {
		uint32_t ip1 = (flowkey >> 96) & 0xFFFFFFFF; // Extract ip1
		uint32_t ip2 = (flowkey >> 64) & 0xFFFFFFFF; // Extract ip2
		uint16_t port1 = (flowkey >> 32) & 0xFFFF; // Extract port1
		uint16_t port2 = flowkey & 0xFFFF; // Extract port2
		return std::make_tuple(ip1, ip2, port1, port2);
	}

	bool SwitchMmu::ShouldSendECN(uint32_t ifindex, uint32_t qIndex){
		if (qIndex == 0) // Means not using ECN
			return false;
		if (egress_bytes[ifindex][qIndex] > kmax[ifindex]) // Already exceed the max threshold
			return true;
		if (egress_bytes[ifindex][qIndex] > kmin[ifindex]){ // Between min and max threshold
	...
	bool SwitchMmu::ShouldSendECN_Customized(uint32_t ifindex, uint32_t qIndex){
		if (qIndex == 0) // Means not using ECN
			return false;
		if (egress_bytes[ifindex][qIndex] > kmax[ifindex]) // Already exceed the max threshold
			return true;

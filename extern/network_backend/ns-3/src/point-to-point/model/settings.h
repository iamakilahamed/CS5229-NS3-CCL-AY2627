...
class Settings {
   public:
    ...
    static uint32_t ecn_mode;
    static uint32_t queue_mgmt_mode;
    ...
    // the map bwteen host's IP and its ToR switch ID
    static std::map<uint32_t, uint32_t> hostIp2SwitchId;

    static uint32_t dropped_pkt_sw_ingress;
    static uint32_t dropped_pkt_sw_egress;

    static bool record_every_ecn;
    static uint32_t ecn_count; // Recording is done by RDMA Hw
    static uint32_t pause_count; // Recording is done by PFC Sender Switch MMU

    // Switch workload statistics
    static std::map<uint32_t, uint64_t> switch_workload_statistics;
    // ECN count statistics
    static std::map<uint32_t, uint64_t> ecn_count_per_switch;
    // Switch workload per switch per port
    static std::map<uint32_t, std::map<uint32_t, uint64_t>> switch_workload_statistics_per_port_ingress;
    static std::map<uint32_t, std::map<uint32_t, uint64_t>> switch_workload_statistics_per_port_egress;
    // ECN count per switch per port
    static std::map<uint32_t, std::map<uint32_t, uint64_t>> ecn_count_per_switch_per_port;

    static FILE *per_flow_ecn_record_output;
    static FILE *rdma_recovery_record_output;

    static uint32_t m_ackHighPrio;

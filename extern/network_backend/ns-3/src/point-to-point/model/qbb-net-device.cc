...
namespace ns3 {
	...
	int RdmaEgressQueue::GetNextQindex(bool paused[]){
		...
		for (qIndex = 1; qIndex <= fcount; qIndex++){
			...
			// 	std::cout << "qp on " << " is paused, bytes left: " << qp->GetBytesLeft() << std::endl;
			// }
			// if (!paused[qp->m_pg] && qp->GetBytesLeft() > 0 && qp->IsWinBound()){
			// 	std::cout << "qp on " << " is win bound, bytes left: " << qp->GetBytesLeft() << std::endl;
			// }
			if (!paused[qp->m_pg] && qp->GetBytesLeft() > 0 && !qp->IsWinBound()){
	...
	void QbbNetDevice::DequeueAndTransmit(void) {
		...
		if (m_node->GetNodeType() == 0) { // Host, using RDMA Equeue
			...
			// In case you want to use customized queueing strategy
			if (Settings::queue_mgmt_mode == 0) { // The default Round Robin Queueing
				
				p = m_queue->DequeueRR(m_paused);

			} else if (Settings::queue_mgmt_mode == 1) { // Customized Queueing
				
				p = m_queue->DequeueCustomized();

			} else {
				NS_ASSERT_MSG(false, "Unknown queue management mode");
				std::cout << "Queue Management Mode " << Settings::queue_mgmt_mode << " Unknown" << std::endl;
				exit(1);
			}
			...
			if (p){
				...
				// This actually can never happen... 
				NS_LOG_INFO("PAUSE prohibits send at node " << m_node->GetId());
		...
		if (m_node->GetNodeType() == 0){
			...
			while (1){
				...
				if (!p)
					 break;
				uint32_t qIndex = m_queue->GetLastQueue();
				// Give the buffer credit back to the MMU before dropping, exactly as the
				// normal dequeue path in DequeueAndTransmit() does. Without this the
				// ingress/egress/shared byte counters are over-counted for good, and a
				// single link failure can push the switch into permanent admission
				// failure on ports that are still healthy.
				m_node->SwitchNotifyDequeue(m_ifIndex, qIndex, p);
				m_traceDrop(p, qIndex);
			}

...
// A queue for sending messages waiting to be sent.
//    - key: <<src_id, dst_id>, port_number>
//    - value: <<tag, maxPktCount>, MsgEvent>
map<pair<pair<int, int>, uint32_t>, pair<pair<int, int>, MsgEvent>> waiting_to_send_events;

// On-going Flow records
//  - key: src_id
//  - value: list of pair<dst_id, port_number>
map<int, vector<pair<int, uint32_t>>> on_going_flows;

void trigger_send(int src_id, int dst, uint32_t port, int dport, int pg, int maxPacketCount, void (*msg_handler)(void *fun_arg), void *fun_arg, int tag) {
...
void trigger_waiting_send(int src_id, int dst) {
  
  int num_flows = 0;
  for (auto it = sim_send_waiting_hash.begin(); it != sim_send_waiting_hash.end(); ++it) {
    if (it->first.first.second.first == src_id && it->first.first.second.second == dst) {
      num_flows++;
    }
  }

  if (num_flows == 0) {
    // No active flows, find a waiting message to send
    // This message should be the one with smallest port number in waiting to send
    uint32_t smallest_port_number = 0xFFFFFFFF;
    int num_waiting_flows = 0;
    for (auto it = waiting_to_send_events.begin(); it != waiting_to_send_events.end(); ++it) {
      if (it->first.first.first == src_id && it->first.first.second == dst) {
        num_waiting_flows++;
        if (it->first.second < smallest_port_number) {
          smallest_port_number = it->first.second;
        }
      }
    }

    if (num_waiting_flows == 0) {

      // No waiting messages, exit
      return;

    } else {
      // There are waiting messages, find the one with smallest port number
      auto it = waiting_to_send_events.find(make_pair(make_pair(src_id, dst), smallest_port_number));
      if (it != waiting_to_send_events.end()) {
        int tag = it->second.first.first;
        int maxPktCount = it->second.first.second;
        MsgEvent send_event = it->second.second;

        waiting_to_send_events.erase(it);

        pair<MsgEventKey, int> send_event_key = make_pair(make_pair(tag, make_pair(send_event.src_id, send_event.dst_id)), smallest_port_number);
        sim_send_waiting_hash[send_event_key] = send_event;
        // Send the message
        trigger_send(src_id, dst, smallest_port_number, 100, 3, maxPktCount, send_event.msg_handler, send_event.fun_arg, tag);
      }
    }
  } else {

    // There are active flows, schedule a check later
    ns3::Simulator::Schedule(
        ns3::NanoSeconds(100),
        &trigger_waiting_send, src_id, dst);
...
void send_flow(int src_id, int dst, int maxPacketCount,
               void (*msg_handler)(void *fun_arg), void *fun_arg, int tag, bool allow_overlapping) {
  // Get a new port number.
  uint32_t port = portNumber[src_id][dst]++;
  sender_src_port_map[make_pair(port, make_pair(src_id, dst))] = tag;
  int pg = 3, dport = 100; // Where the PG Priority Group and Dport are hardcoded values.
  flow_input.idx++;

  // Create a MsgEvent instance and register callback function.
  MsgEvent send_event = MsgEvent(src_id, dst, 0, maxPacketCount, fun_arg, msg_handler);
  pair<MsgEventKey, int> send_event_key = make_pair(make_pair(tag, make_pair(send_event.src_id, send_event.dst_id)),port);

  if (!allow_overlapping) {
    
    // Check if there is an active flow from src_id to dst.
    int num_flows = 0;
    for (auto it = sim_send_waiting_hash.begin(); it != sim_send_waiting_hash.end(); ++it) {
      if (it->first.first.second.first == src_id && it->first.first.second.second == dst) {
        num_flows++;
    ...
    if(num_flows == 0) {
      ...
      if (num_waiting_flows == 0) {
        ...
        // There is no active flows, no waiting messages, send the message immediately.
        sim_send_waiting_hash[send_event_key] = send_event;
        trigger_send(src_id, dst, port, dport, pg, maxPacketCount, msg_handler, fun_arg, tag);
      
      } else {

        // There is no active flows, but there are waiting messages, need to queue for that
        waiting_to_send_events[make_pair(make_pair(src_id, dst), port)] = make_pair(make_pair(tag, maxPacketCount), send_event);
        trigger_waiting_send(src_id, dst);
      ...
      // There is one active flows, put current message in the queue
      waiting_to_send_events[make_pair(make_pair(src_id, dst), port)] = make_pair(make_pair(tag, maxPacketCount), send_event);
      trigger_waiting_send(src_id, dst);

    } else if (num_flows > 1) {
      // There are multiple active flows, something is wrong under this configuration,
      // Exit the program with error message.
      std::cerr << "There are multiple active flows from " << src_id << " to " << dst << std::endl;
      exit(1);
    } else {
      std::cerr << "should not reach here, something is wrong." << std::endl;
      std::cerr << "src_id: " << src_id << ", dst: " << dst << ", num_flows: " << num_flows << std::endl;
      exit(1);
    }
  } else {
    // If allow_overlapping is true, we can send the message immediately.
    sim_send_waiting_hash[send_event_key] = send_event;
    // Record the flow in on_going_flows.
    if (on_going_flows.find(src_id) == on_going_flows.end()) {
      on_going_flows[src_id] = vector<pair<int, uint32_t>>();
    }
    on_going_flows[src_id].push_back(make_pair(dst, port));
    // std::cout << "Sending flow from " << src_id << " to " << dst << " with port " << port << std::endl;
    trigger_send(src_id, dst, port, dport, pg, maxPacketCount, msg_handler, fun_arg, tag);
  }

  // If there are multiple flows from the same source to the same destination,
  // Print the situation out.
  // int num_flows = 0;
  // for (auto it = sim_send_waiting_hash.begin(); it != sim_send_waiting_hash.end(); ++it) {
  //   if (it->first.first.second.first == src_id && it->first.first.second.second == dst) {
  //     num_flows++;
  ...
  // }
  // if (num_flows > 1) {
  //   std::cout << "Multiple flows from " << src_id << " to " << dst << ": ";
  //   for (auto it = sim_send_waiting_hash.begin(); it != sim_send_waiting_hash.end(); ++it) {
  //     if (it->first.first.second.first == src_id && it->first.first.second.second == dst) {
...
void print_on_going_flows() {
  if (on_going_flows.empty()) {
    
  } else {
    std::cout << "On-going flows: " << endl;
    for (const auto &flow : on_going_flows) {
      std::cout << "Source ID: " << flow.first << ", Flows: ";
      for (const auto &dst_port : flow.second) {
        std::cout << "(" << dst_port.first << ", " << dst_port.second << ") ";
      }
      std::cout << endl;
    }
  }
  // Schedule the next print
  ns3::Simulator::Schedule(ns3::NanoSeconds(1000), &print_on_going_flows);
}

void notify_sender_sending_finished(int src_id, int dst_id, int message_size,
                                    int tag, int src_port) {

  // Update on_going_flows.
  if (on_going_flows.find(src_id) != on_going_flows.end()) {
    auto &flows = on_going_flows[src_id];
    for (auto it = flows.begin(); it != flows.end(); ++it) {
      if (it->first == dst_id && it->second == src_port) {
        flows.erase(it);
        break;
      }
    }
    if (flows.empty()) {
      on_going_flows.erase(src_id);
    }
  }
...
// finished. qp_finish is registered as the callback handler to the RdmaClient
...
int setup_ns3_simulation(string network_configuration, string workload_name, string system_name, string network_name, string topo_name, bool allow_overlapping, string comm_group_name) {
  if (!ReadConf(network_configuration))
    return -1;
  SetConfig();
  // std::string workload_name, std::string system_name, std::string network_name, std::string topo_name
  SetupNetwork(qp_finish, workload_name, system_name, network_name, topo_name, allow_overlapping, comm_group_name);
  // print_on_going_flows();
  return 0;
}

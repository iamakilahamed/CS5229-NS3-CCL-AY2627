...
Workload::Workload(Sys* sys, string et_filename, string comm_group_filename, FILE* report_record_file) {
    ...
    this->report_record_file = report_record_file;
}
...
void Workload::initialize_comm_group(string comm_group_filename) {
    ...
    for (json::iterator it = j.begin(); it != j.end(); ++it) {
        ...
        if (in_comm_group) {
            ...
            // Use the group id from the JSON key rather than a hard-coded 1.
            // CommunicatorGroup::set_id() derives the stream-id namespace from the id
            // (num_streams = id * 1000000), so a hard-coded 1 makes every concurrent
            // group share one stream-id space. That is what stops several independent
            // jobs from running at the same time.
            // Note: All NPUs should create comm group with identical ids if they want
            // to communicate with each other -- which holds here, since every NPU reads
            // the same comm-group file and keys off the same group id.
            int group_id = std::stoi(it.key());
            comm_group = new CommunicatorGroup(group_id, involved_NPUs, sys);
        }
    }
}
...
void Workload::issue(shared_ptr<Chakra::ETFeederNode> node) {
    if (sys->replay_only) {
        ...
        if ((node->type() == ChakraNodeType::MEM_LOAD_NODE) || (node->type() == ChakraNodeType::MEM_STORE_NODE)) {
            if (sys->trace_enabled) {
                cout << "issue,sys->id=" << sys->id << ",tick=" << Sys::boostedTick() << ",node->id=" << node->id()
        ...
        } else if (!node->is_cpu_op() && (node->type() == ChakraNodeType::COMM_COLL_NODE ||
...
void Workload::issue_comp(shared_ptr<Chakra::ETFeederNode> node) {
    ...
    if (sys->roofline_enabled) {
        WorkloadLayerHandlerData* wlhd = new WorkloadLayerHandlerData;
        wlhd->node_id = node->id();
        // These are to calcuate how long this node will take to finish
...
void Workload::issue_comm(shared_ptr<Chakra::ETFeederNode> node) {
    ...
    // For send and receive, don't need to check the involved dimension
    // For collective communication, we need to check the involved dimension
    ...
    // corner case: involved dim is not defined, just put one true, anyway it's not used
    ...
    if (!node->is_cpu_op() && (node->type() == ChakraNodeType::COMM_COLL_NODE)) {
        if (node->comm_type() == ChakraCollectiveCommType::ALL_REDUCE) {
            DataSet* fp = sys->generate_all_reduce(node->comm_size(), involved_dim, comm_group, node->comm_priority());
            ...
            DataSet* fp = sys->generate_all_to_all(node->comm_size(), involved_dim, comm_group, node->comm_priority());
            collective_comm_node_id_map[fp->my_id] = node->id();
            collective_comm_wrapper_map[fp->my_id] = fp;
            fp->set_notifier(this, EventType::CollectiveCommunicationFinished);

        } else if (node->comm_type() == ChakraCollectiveCommType::ALL_GATHER) {
            DataSet* fp = sys->generate_all_gather(node->comm_size(), involved_dim, comm_group, node->comm_priority());
            collective_comm_node_id_map[fp->my_id] = node->id();
            collective_comm_wrapper_map[fp->my_id] = fp;
            fp->set_notifier(this, EventType::CollectiveCommunicationFinished);
            ...
            DataSet* fp =
                sys->generate_reduce_scatter(node->comm_size(), involved_dim, comm_group, node->comm_priority());
            collective_comm_node_id_map[fp->my_id] = node->id();
            collective_comm_wrapper_map[fp->my_id] = fp;
            fp->set_notifier(this, EventType::CollectiveCommunicationFinished);
        ...
        sys->front_end_sim_send(0, Sys::dummy_data, node->comm_size(), UINT8, node->comm_dst(), node->comm_tag(),
                                &snd_req, &Sys::handleEvent, sehd);
        ...
        sys->front_end_sim_recv(0, Sys::dummy_data, node->comm_size(), UINT8, node->comm_src(), node->comm_tag(),
                                &rcv_req, &Sys::handleEvent, rcehd);
    } else {
...
// start the events in the workloads, basically the entreance of the workloads
...
void Workload::call(EventType event, CallData* data) {
    ...
    if (event == EventType::CollectiveCommunicationFinished) {
        ...
        if (sys->trace_enabled) {
            cout << "callback,sys->id=" << sys->id << ",tick=" << Sys::boostedTick() << ",node->id=" << node->id()
                 << ",node->name=" << node->name() << endl;
        }
        ...
        if (data == nullptr) { 
            // Fire the event on the host, is this situation:
            // EventType: GENERAL, data: NULL
            // Find a task that has no dependency and run it. 
            issue_dep_free_nodes();
        } else {
    ...
    if (!et_feeder->hasNodesToIssue() && (hw_resource->num_in_flight_cpu_ops == 0) &&
        (hw_resource->num_in_flight_gpu_comp_ops == 0) && (hw_resource->num_in_flight_gpu_comm_ops == 0)) {
        report();
        is_finished = true;
    }
}
...
// Host finish work recording
void Workload::report() {
    Tick curr_tick = Sys::boostedTick();
    cout << "sys[" << sys->id << "] finished, " << curr_tick << " cycles" << endl;
    fprintf(report_record_file, "sys[%d] finished, %lu cycles\n", sys->id, curr_tick);
}
